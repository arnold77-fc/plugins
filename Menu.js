(function() {    
    ...
    function startPlugin() {    
        ...
        function initialize() {    
            // Внедрение принудительных стилей скрытия (перекрывает CSS AgNative)
            ...
            function applyLeftMenu() {    
                ...
                $('.menu .menu__item').removeClass('hidden hide');    
            ...
            function applyTopMenu() {    
                ...
                $('.head__action').removeClass('hide hidden');    
                if(hide.length) {    
                    hide.forEach((uniqueClass) => {    
                        let item = $('.head__action.' + uniqueClass);    
                        if(item.length) item.addClass('hide hidden');    
                    });    
                }    
            }    
            ...
            function applySettingsMenu() {    
                ...
                $('.settings-folder').removeClass('hide hidden');    
                if(hide.length) {    
                    hide.forEach((name) => {    
                        let item = $('.settings-folder').filter(function() {    
                            return $(this).find('.settings-folder__name').text().trim() === name;    
            ...
            // Функция получения имени пункта верхнего меню    
            ...
            function editLeftMenu() {    
                ...
                menu.find('.menu__item').each(function(){    
                    ...
                    item_sort.find('.toggle').on('hover:enter', ()=>{    
                        item_orig.toggleClass('hidden hide');    
                        item_sort.find('.dot').attr('opacity', item_orig.hasClass('hidden') || item_orig.hasClass('hide') ? 0 : 1);    
                    }).find('.dot').attr('opacity', item_orig.hasClass('hidden') || item_orig.hasClass('hide') ? 0 : 1);    
            ...
            function editTopMenu() {    
                ...
                head.find('.head__action').each(function(){    
                    ...
                    item_sort.find('.toggle').on('hover:enter', ()=>{  
                        item_orig.toggleClass('hide hidden');  
                        item_sort.find('.dot').attr('opacity', item_orig.hasClass('hide') || item_orig.hasClass('hidden') ? 0 : 1);  
                    }).find('.dot').attr('opacity', item_orig.hasClass('hide') || item_orig.hasClass('hidden') ? 0 : 1);  
                ...
                if (Lampa.Storage.get('add_clear_cache_button', false)) {  
                    ...
                    clearItem.find('.toggle').on('hover:enter', ()=>{  
                        $('.head__action--clear-cache').toggleClass('hide hidden');  
                        clearItem.find('.dot').attr('opacity', $('.head__action--clear-cache').hasClass('hide') || $('.head__action--clear-cache').hasClass('hidden') ? 0 : 1);  
                    }).find('.dot').attr('opacity', $('.head__action--clear-cache').hasClass('hide') || $('.head__action--clear-cache').hasClass('hidden') ? 0 : 1);  
                ...
                if (Lampa.Storage.get('add_reload_button', false)) {  
                    ...
                    reloadItem.find('.toggle').on('hover:enter', ()=>{  
                        $('.head__action--reload').toggleClass('hide hidden');  
                        reloadItem.find('.dot').attr('opacity', $('.head__action--reload').hasClass('hide') || $('.head__action--reload').hasClass('hidden') ? 0 : 1);  
                    }).find('.dot').attr('opacity', $('.head__action--reload').hasClass('hide') || $('.head__action--reload').hasClass('hidden') ? 0 : 1);  
            ...
            function saveLeftMenu() {  
                ...
                $('.menu .menu__item').each(function(){  
                    if($(this).hasClass('hidden') || $(this).hasClass('hide')){  
                        let name = $(this).find('.menu__text').text().trim();  
                        hide.push(name);  
                    }  
                });  
            ...
            // Добавление компонентов в настройки  
            ...
            // Запуск применения с задержками для перекрытия асинхронной загрузки AgNative
            ...
            // Динамический наблюдатель изменений DOM (для синхронизации с AgNative)
