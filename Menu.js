(function () {
    ...
    function startPlugin() {
        ...
        function initialize() {
            ...
            // ==========================================
            // 1. ИНЪЕКЦИЯ СТИЛЕЙ С МАКСИМАЛЬНЫМ ПРИОРИТЕТОМ
            // ==========================================
            function injectStyles() {
                ...
                $('head').append(`
                    <style id="menu_editor_style">
                        body .menu .menu__item.me-hidden,
                        body .head .head__action.me-hidden,
                        body .settings-folder.me-hidden,
                        body .navigation-bar.me-hidden,
                        .menu .menu__item.me-hidden,
                        .menu .menu__item.hide,
                        .menu .menu__item.hidden,
                        .head__action.me-hidden,
                        .head__action.hide,
                        .head__action.hidden,
                        .settings-folder.me-hidden,
                        .settings-folder.hide,
                        .settings-folder.hidden {
            ...
            // ==========================================
            // 2. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ СКРЫТИЯ/ПОКАЗА DOM
            // ==========================================
            function hideEl($el) {
                if (!$el || !$el.length) return;
                $el.addClass('me-hidden hide hidden');
                $el.each(function () {
            ...
            function showEl($el) {
                if (!$el || !$el.length) return;
                $el.removeClass('me-hidden hide hidden');
                $el.each(function () {
            ...
            // ==========================================
            // 4. ПРИМЕНЕНИЕ НАСТРОЕК ЛЕВОГО МЕНЮ
            // ==========================================
            function applyLeftMenu() {
                ...
                if (sort.length) {
                    sort.forEach((name) => {
                        ...
                        if (item.length) {
                            item.appendTo(menu.find('.menu__list:eq(0)'));
                        }
                    });
                }
                ...
                if (hide.length) {
                    hide.forEach((name) => {
                        let item = menu.find('.menu__item').filter(function () {
                            return $(this).find('.menu__text').text().trim() === name;
                        });
                        if (item.length) {
                            hideEl(item);
            ...
            // ==========================================
            // 5. ПРИМЕНЕНИЕ НАСТРОЕК ВЕРХНЕГО МЕНЮ (HEAD)
            // ==========================================
            ...
            // ==========================================
            // 6. ПРИМЕНЕНИЕ НАСТРОЕК МЕНЮ НАСТРОЕК
            // ==========================================
            function applySettingsMenu() {
                ...
                if (sort.length && settingsContainer.length) {
                    sort.forEach((name) => {
                        ...
                        if (item.length) {
                            item.appendTo(settingsContainer);
                        }
                    });
                }
            ...
            // ==========================================
            // 7. СКУПНОЕ ОБНОВЛЕНИЕ ВСЕХ ПАНЕЛЕЙ И NAVIGATION BAR
            // ==========================================
            function applyNavBar() {
                if (Lampa.Storage.field('hide_navigation_bar') == true) {
                    hideEl($('.navigation-bar'));
                } else {
                    showEl($('.navigation-bar'));
                }
            }
            ...
            // ==========================================
            // 8. ОКНО РЕДАКТИРОВАНИЯ ЛЕВОГО МЕНЮ
            // ==========================================
            function editLeftMenu() {
                ...
                menu.find('.menu__item').each(function () {
                    ...
                    let isHidden = item_orig.hasClass('me-hidden') || item_orig.hasClass('hide') || item_orig.hasClass('hidden');
                    item_sort.find('.dot').attr('opacity', isHidden ? 0 : 1);

                    item_sort.find('.toggle').on('hover:enter', () => {
                        if (item_orig.hasClass('me-hidden') || item_orig.hasClass('hide') || item_orig.hasClass('hidden')) {
                            showEl(item_orig);
                        } else {
                            hideEl(item_orig);
                        }
                        let nowHidden = item_orig.hasClass('me-hidden') || item_orig.hasClass('hide') || item_orig.hasClass('hidden');
                        item_sort.find('.dot').attr('opacity', nowHidden ? 0 : 1);
                    });
            ...
            // ==========================================
            // 9. ОКНО РЕДАКТИРОВАНИЯ ВЕРХНЕГО МЕНЮ
            // ==========================================
            function editTopMenu() {
                ...
                head.find('.head__action').each(function () {
                    let item_orig = $(this);
                    let item_clone = $(this).clone();
                    let allClasses = item_clone.attr('class') ? item_clone.attr('class').split(' ') : [];
            ...
            // ==========================================
            // 10. ОКНО РЕДАКТИРОВАНИЯ МЕНЮ НАСТРОЕК
            // ==========================================
            ...
            // ==========================================
            // 11. СОХРАНЕНИЕ НАСТРОЕК В STORAGE
            // ==========================================
            function saveLeftMenu() {
                ...
                $('.menu .menu__item').each(function () {
                    if ($(this).hasClass('me-hidden') || $(this).hasClass('hidden') || $(this).hasClass('hide')) {
                        let name = $(this).find('.menu__text').text().trim();
                        hide.push(name);
                    }
                });
            ...
            function saveTopMenu() {
                ...
                $('.head__action').each(function () {
                    ...
                    if (uniqueClass) {
                        ...
                        if ($(this).hasClass('me-hidden') || $(this).hasClass('hide') || $(this).hasClass('hidden')) {
            ...
            // ==========================================
            // 12. РЕГИСТРАЦИЯ ПЛАГИНА В НАСТРОЙКАХ LAMPA
            // ==========================================
            ...
            // ==========================================
            // 13. НАБЛЮДАТЕЛЬ ЗА DOM ДЛЯ ОБХОДА AGNATIVE
            // ==========================================
            ...
            let observer = new MutationObserver((mutations) => {
                ...
                for (let i = 0; i < mutations.length; i++) {
                    ...
                    if (target && target.closest) {
                        if (target.closest('.head') || target.closest('.menu') || target.closest('.settings')) {
                            shouldReapply = true;
                            break;
                        }
                    }
            ...
            // Фоновый интервал на случай пропуска событий
            setInterval(applyAll, 1000);

            // Слушатели системных событий Lampa
            Lampa.Listener.follow('menu', (e) => {
                if (e.type === 'end' || e.type === 'open') {
                    setTimeout(applyLeftMenu, 50);
                }
            });
            ...
            if (Lampa.Settings && Lampa.Settings.listener) {
                Lampa.Settings.listener.follow('open', (e) => {
                    setTimeout(applySettingsMenu, 250);
                });
            }
        }
    ...
    if (!window.plugin_menu_editor_ready) {
        startPlugin();
    }
})();
