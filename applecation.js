...
.applecation .items-line:has(.warning-box--full-adult) .items-line__body {
    ...
    Object.assign(translations, {
        status_sequel: { ru: 'Продолжается', uk: 'Продовжується', en: 'Returning Series', be: 'Працягваецца', bg: 'Продължава', cs: 'Pokračování', he: 'סִקְוֶל', pt: 'Sequência', zh: '续集' },
        status_ended: { ru: 'Завершился', uk: 'Завершився', en: 'Ended', be: 'Скончыўся', bg: 'Приключил', cs: 'Ukončeno', he: 'הסתיים', pt: 'Terminado', zh: '已完结' },
        status_canceled: { ru: 'Отменен', uk: 'Скасовано', en: 'Canceled', be: 'Скасавана', bg: 'Отменен', cs: 'Zrušeno', he: 'בוטל', pt: 'Cancelado', zh: '已取消' },
        status_tomorrow: { ru: 'Завтра серия', uk: 'Завтра серія', en: 'Tomorrow', be: 'Заўтра серыя', bg: 'Утре', cs: 'Zítra', he: 'מחר', pt: 'Amanhã', zh: '明天' },
        status_in_week: { ru: 'Через неделю', uk: 'Через тиждень', en: 'In a week', be: 'Праз тыдзень', bg: 'След седмица', cs: 'Za týden', he: 'בעוד שבוע', pt: 'Em uma semana', zh: '一周后' },
        status_in_days: { ru: 'Через %d дн.', uk: 'Через %d дн.', en: 'In %d days', be: 'Праз %d дн.', bg: 'След %d дни', cs: 'Za %d dn.', he: 'בעוד %d ימים', pt: 'Em %d dias', zh: '%d天后' }
    });

    // === ФУНКЦИЯ ОПРЕДЕЛЕНИЯ СТАТУСА (ТОЧНЫЙ РАСЧЕТ) ===
    function getSeriesStatusText(data) {
        if (!data || !data.name) return null;
        const status = data.status;
        const nextEpisode = data.next_episode_to_air;
        let statusText = '';

        if (status === 'Returning Series' || status === 'In Production') statusText = t('status_sequel');
        else if (status === 'Ended') statusText = t('status_ended');
        else if (status === 'Canceled' || status === 'Cancelled') statusText = t('status_canceled');

        if (nextEpisode && nextEpisode.air_date) {
            try {
                // Разбираем дату YYYY-MM-DD напрямую
                const parts = nextEpisode.air_date.split('-');
                const nextAirDate = new Date(parts[0], parts[1] - 1, parts[2]);
                
                // Берем текущую дату без времени (полночь)
                const now = new Date();
                const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());

                // Считаем разницу в днях
                const diffTime = nextAirDate.getTime() - today.getTime();
                const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24));

                if (diffDays > 0 && diffDays <= 31) {
                    if (diffDays === 1) statusText = t('status_tomorrow');
                    else if (diffDays === 7) statusText = t('status_in_week');
                    else statusText = t('status_in_days').replace('%d', diffDays);
                }
            } catch (e) {
                console.log('Error calculating date:', e);
            }
        }
        return statusText;
    }
    ...
    function fillAdditionalInfo(activity, data) {
        ...
        if (data.name) {
            ...
            if (Lampa.Storage.get('applecation_show_episode_count', false)) {
                if (data.number_of_episodes) infoParts.push(formatEpisodes(data.number_of_episodes));
            }
        } else {
        ...
        // 3. Статус в самом конце
        if (data.name) {
            const statusText = getSeriesStatusText(data);
            if (statusText) infoParts.push(statusText);
        }

        infoContainer.html(infoParts.join(' · ') + '<span class="applecation__quality-badges"></span>');
    }
