(function () {
  'use strict';
  /* --- GOOGLE TV PERFORMANCE OPTIMIZATION MODULE --- */
  var AGNATIVE_TV_PERF = {
    // Принудительное включение аппаратного ускорения для всех карточек и списков
    applyGpuAcceleration: function() {
      var styleId = 'agnative-tv-gpu-fix';
      if (document.getElementById(styleId)) return;
      
      var style = document.createElement('style');
      style.id = styleId;
      style.innerHTML = [
        /* Переводим отрисовку в GPU, чтобы разгрузить слабый процессор приставки */
        '.card, .menu__item, .settings__item, .selectbox, .scroll__body {',
        ...
        '}',
        /* Убираем тяжелые тени и размытия (blur), которые вешают Google TV */
        'body[data-agnative-perf="ultra"] .card__view,',
        'body[data-agnative-perf="ultra"] .head,',
        'body[data-agnative-perf="ultra"] .modal {',
    ...
    // Оптимизатор частоты кадров и очистки памяти при скролле
    initMemoryCleaner: function() {
      var lastCleanTime = Date.now();
      window.addEventListener('scroll', function() {
        var now = Date.now();
        // Очищаем неиспользуемые таймеры и изображения раз в 30 секунд активного скролла
    ...
    init: function() {
      ...
      this.initMemoryCleaner();
      console.log('AppleTV AgNative: Google TV GPU & RAM optimization loaded.');
    }
  };

  // Автоматический запуск при старте плагина
  if (typeof document !== 'undefined') {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function() { AGNATIVE_TV_PERF.init(); });
    } else {
      AGNATIVE_TV_PERF.init();
    }
  }
